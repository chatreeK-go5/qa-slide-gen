...
Slidev (sli.dev) — renders Vue + SVG slides via Playwright
      ↓
GitHub Actions Artifact  (qa-charts-{run_id}.zip, retained 30 days)
```
...
│   └── DonutPair.vue            # N-donut chart, scales for 2+ charts (Summary by Priority)
│
├── scripts/
...
> **Note:** `artifacts/` and `data/current/` are both `.gitignore`d.
> PNGs are uploaded as GitHub Actions artifacts — not committed to the repo.
...
|---|------|-----------|-------------|
| 1 | `production_issues.png` | Horizontal bar | Status-grouped counts with large total badge |
...
| 4 | `summary_by_priority.png` | Donut charts (3) | Beauty · PI WEB · PI Mobile, priority breakdown |
...
Generated PNGs are uploaded as a **GitHub Actions artifact** named
`qa-charts-{run_id}` and retained for **30 days**. Download from:

```
https://github.com/{owner}/{repo}/actions/runs/{run_id}
```

The repository itself is never modified by CI (`contents: read` permission only).
...
# Live development preview (requires data/current/ to exist)
npm run dev
```

PNGs are written locally to `artifacts/YYYY-MM-DD/` (gitignored).
...
```json
{
  ...
  "charts": [
    ...
    },
    {
      ...
      "total": 181,
      "priorities": [
        { "label": "Highest", "count": 31  },
        { "label": "High",    "count": 36  },
        { "label": "Medium",  "count": 114 }
      ]
    },
    {
      "label": "PI Mobile=47",
...
> - `charts[]` order determines left→right rendering; supports 2 or 3 entries
